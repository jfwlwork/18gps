...
<template>
  <div class="ant-pro-page-container" :class="{ 'ant-pro-page-container-occupied': props.occupied }">
    <div class="bg-[var(--bg-color)]" :class="layoutSetting.multiTab ? 'pb-16px' : 'py-8px'" px-24px mb-24px mx--24px mt--24px>
      <a-breadcrumb v-if="!currentItem.hideInBreadcrumb">
        <template v-if="currentItem.matched?.length">
          <a-breadcrumb-item v-for="item in currentItem.matched" :key="item.path">
            <router-link v-if="linkable" :to="item.path">
...
<style>
.ant-pro-page-container-occupied{
  width: 100%;
  height: 100%;
}
</style>
